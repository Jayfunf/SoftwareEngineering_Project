...
public class CV_Controller {
    ...
    public Bitmap CV_start(Mat inputmat){

        //Utils.bitmapToMat(); << bit to mat
        //Utils.matToBitmap(); << mat to bit

        //Mat inputmat = Imgcodecs.imread("takepicture.jpg");
        Mat cont = Contour(inputmat);
        // 이 cleaner 이라는 파일형식이 opencv_contour_img.jpg 파일입니다.
        Bitmap cleaner = cleanimg(cont); // 이 파일을 Tesseract 에 넣어주기만 하면 됩니다.

        return cleaner;

    }

    private static Bitmap cleanimg(Mat input) {
        ...
        Bitmap resultFile = BitmapFactory.decodeFile("opencv_contour_img.jpg");
